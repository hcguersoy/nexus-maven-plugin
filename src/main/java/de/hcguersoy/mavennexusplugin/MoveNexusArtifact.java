...
/**
 * This plugin moves an artifact in Nexus OSS from one repository to another repository.
 * For this, it uses the Nexus REsT API.
 * 
 * The dependency and deployment plugins are needed to download 
 * an artifact from the origin repository and to upload it into the 
 * target repository.
 * 
...
public class MoveNexusArtifact extends AbstractMojo {
	...
	/**
	 * The packaging of the artifact to be moved.
	 * Be aware that we allways will move the according POM, too.
	 */
	@Parameter (required = true)
	...
	/**
	 * The Root URL of the target Repository Server , e.g. ${nexusURL}:${nexusPort}.
	 * This parameter is optional, if not provided we'll take the same URL as for stagingNexus.
	 */
	...
	/**
	 * The path to the temporary saved artifact. This is needed 
	 * during the "move" of the artifact.
	 */
	...
	/**
	 * The maven-deploy-plugin Version. Needed to upload the
	 * artifact into the target repository.
	 */
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		...
		if (targetNexus == null) targetNexus = stagingNexus;
		...
		if (skipGetArtifacts.equals(DONT_SKIP_STEP))	{getArtifacts();}   else {getLog().debug("Skipping getArtifacts");};
		if (skipDeployArtifacts.equals(DONT_SKIP_STEP)) {deployArtifact();} else {getLog().debug("Skipping deployArtifact");};
		if (skipDeleteArtifacts.equals(DONT_SKIP_STEP)) {deleteArtifact();} else {getLog().debug("Skipping deleteArtifact");};
	}
	...
	/**
	 * This method fetches the artifact using the maven dependency plugin.
	 * To call this plugin, we use the 'mojo-executor' plugin 
	 * (see http://timmoore.github.io/mojo-executor/).
	 * 
	...
	/**
	 * Deploys the artifact and its POM in the target repository.
	 * Here we use the maven deployment plugin.
	 * 
	 * @throws MojoExecutionException
	 * @throws MojoFailureException
	 */
	...
	/**
	 * After deploying the artifact in the target repository we've to delete it from
	 * the origin repository. This is done using the Nexus OSS REST API, simply 
	 * calling a HTTP DELETE on the resource.
	 *   
	 * @throws MojoExecutionException
	 * @throws MojoFailureException
	 */
	private void deleteArtifact() throws MojoExecutionException, MojoFailureException {
		...
		String requestURL = targetNexus + NEXUS_CONTENT_PATH + stagingRepoID + DELI + groupId.replace(".", DELI) + DELI + artifactId + DELI + version + DELI;
